...
## wikipedia [OSI model](https://en.wikipedia.org/wiki/OSI_model)(7层)
...
*物（物理层）→ 链（数据链路层）→ 网（网络层）→ 叔（传输层）→ 会（会话层）→ 使（表示层）→ 用（应用层）*  
此口诀通过谐音和首字对应各层名称，逻辑清晰且口语化，适合快速记忆‌



| 层级名称  | 核心功能             | 典型协议/设备      |
| ----- | ---------------- | ------------ |
| 应用层   | 用户交互接口（如网页浏览、邮件） | HTTP、FTP、DNS |
| 表示层   | 数据格式转换、加密解密      | SSL、JPEG     |
| 会话层   | 建立/管理通信会话        | NetBIOS      |
| 传输层   | 端到端可靠/不可靠传输      | TCP、UDP      |
| 网络层   | IP寻址与路由选择        | IP、路由器       |
| 数据链路层 | MAC地址封装与帧传输      | 以太网、交换机      |
| 物理层   | 物理介质传输比特流        | 网线、光纤、集线器    |

**分层对比TCP/IP模型**‌：OSI的会话层、表示层在TCP/IP模型中合并至应用层，可对比差异强化记忆‌。



## wikipedia [Internet protocol suite](https://en.wikipedia.org/wiki/Internet_protocol_suite)(4层)
...
*物（物理层）→ 联（数据链路层）→ 网（网络层）→ 叔（传输层）→ 应用（应用层）*  
此口诀对应TCP/IP五层模型，通过谐音简化层级名称，便于快速记忆‌

TCP/IP模型功能速览

| 层级名称  | 核心功能           | 典型协议/设备      |
| ----- | -------------- | ------------ |
| 应用层   | 提供用户服务（如网页、邮件） | HTTP、FTP、DNS |
| 传输层   | 端到端可靠/不可靠传输    | TCP、UDP      |
| 网络层   | IP寻址与路由选择      | IP、ICMP、路由器  |
| 数据链路层 | MAC地址封装与帧传输    | 以太网、交换机      |
| 物理层   | 物理介质传输比特流      | 网线、光纤、集线器    |
